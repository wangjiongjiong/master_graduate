...
class GatedSelfAttentionDense(nn.Module):
    def __init__(self, query_dim, context_dim,  n_heads, d_head):
        ...
        self.attn = MemoryEfficientCrossAttention(query_dim=query_dim, heads=n_heads, dim_head=d_head)
...
class GatedSelfAttention(nn.Module):
    def __init__(self, dim, n_heads, d_head, dropout=0.):
        super().__init__()
        self.attn = MemoryEfficientCrossAttention(query_dim=dim, heads=n_heads, dim_head=d_head, dropout=dropout)
        self.token_selection = lambda x: x[:, :x.size(1)//2, :]  # Dummy token selection function
        self.gamma = nn.Parameter(torch.zeros(1))  # Learnable scalar, initialized as 0
        self.beta = 1  # Fixed during training, may vary during inference

    def forward(self, v, he):
        v_he = torch.cat([v, he], dim=1)
        attention_output = self.attn(v_he)
        selected_tokens = self.token_selection(attention_output)
        gated_output = self.beta * torch.tanh(self.gamma) * selected_tokens
        return v + gated_output

class BasicTransformerBlock(nn.Module):
    def __init__(self, dim, n_heads, d_head, dropout=0., context_dim=None, gated_ff=True, checkpoint=True):
        super().__init__()
        self.attn1 = MemoryEfficientCrossAttention(query_dim=dim, heads=n_heads, dim_head=d_head, dropout=dropout)  # is a self-attention
        self.gated_attn = GatedSelfAttentionDense(dim, context_dim, n_heads, d_head)  # 新增的门控自注意力层
        self.ff = FeedForward(dim, dropout=dropout, glu=gated_ff)
        self.attn2 = CrossAttention(query_dim=dim, context_dim=context_dim,
                                    heads=n_heads, dim_head=d_head, dropout=dropout)  # is self-attn if context is none
        self.norm1 = nn.LayerNorm(dim)
        self.norm2 = nn.LayerNorm(dim)
        self.norm3 = nn.LayerNorm(dim)
        self.norm4 = nn.LayerNorm(dim)  # 新增的归一化层
        self.checkpoint = checkpoint
    def forward(self, x, context=None, control=None, mask=None):
        # def forward(self, x, context=None, control=None, mask=None):
        return checkpoint(self._forward, (x, context,control,mask), self.parameters(), self.checkpoint)

    def _forward(self, x, context=None, control=None, mask=None):
        x = self.attn1(self.norm1(x)) + x
        x = self.gated_attn(self.norm2(x), control)
        x = self.attn2(self.norm3(x), context=context, control=control, mask=mask) + x
